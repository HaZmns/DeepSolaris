...
### loading the images
home_dir = '\\Users\\ThinkPad User\\Google Drive\\Master_thesis_H_Z_DL&NN\\Images'
os.chdir(home_dir)
#images_complete = np.load('images_Aachen_complete.npy')
#labels_complete = np.load('labels_Aachen_complete.npy')
images_complete = np.load('images_Aachen_quarter.npy')
labels_complete = np.load('labels_Aachen_quarter.npy')
...
# We know that the images are 130 pixels in each dimension
img_size = 130
...
### Plot a few images to see if the data is correct
    ...
    cls_true = test_labels[0:9].astype(int)
...
logdir = '\\Users\\ThinkPad User\\logs\\'
os.chdir(LOGDIR)
total_iterations = 0 
          ...
          for filter_size1 in fsize1_vector: 
              for filter_size2 in fsize2_vector:
                if (num_filter2 < num_filter1):
                    ...
                    #hparam = make_hparam_string(lr, num_filter1, num_filter2, filter_size1, filter_size2)
